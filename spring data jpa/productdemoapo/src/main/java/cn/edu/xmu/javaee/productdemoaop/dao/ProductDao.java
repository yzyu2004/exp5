...
import cn.edu.xmu.javaee.productdemoaop.repository.ProductPoRepository;
...
public class ProductDao {
    ...
    @Autowired
    ProductPoRepository productPoRepository;


    public List<Product> findProductsByName(String name) {
        List<Product> productList = new ArrayList<>();

        // 使用ProductPoRepository根据名称查询ProductPo列表
        List<ProductPo> productPoList = productPoRepository.findByNameContaining(name);

        for (ProductPo po : productPoList) {
            ProductAllPo productAllPo = new ProductAllPo();
            Product product = CloneFactory.copy(new Product(), po);
            List<OnSale> latestOnSale = onSaleDao.getLatestOnSale(po.getId());
            product.setOnSaleList(latestOnSale);
            List<Product> otherProduct = retrieveOtherProduct(po);
        ...
        logger.debug("findProductsByName: productList = {}", productList);
